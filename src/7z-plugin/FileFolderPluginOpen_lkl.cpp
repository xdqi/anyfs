// FileFolderPluginOpen.cpp (patched for LKL)
...
struct CThreadArchiveOpen {
	...
	*/
	...
	{
		try {
			CProgressCloser closer(
			    OpenCallbackSpec->ProgressDialog);
			Result = FolderManager->OpenFolderFile(
			    InStream, Path, ArcFormat, &Folder,
...
/*
static int FindPlugin(const CObjectVector<CPluginInfo> &plugins, const UString
&pluginName)
{
...
static void GetFolderLevels(CMyComPtr<IFolderFolder>& folder,
			    CArcLevelsInfo& levels)
{
	levels.Levels.Clear();
	...
	for (UInt32 level = 0; level <= numLevels; level++) {
		const PROPID propIDs[] = {kpidError, kpidPath, kpidType,
					  kpidErrorType};
		...
		for (Int32 i = 0; i < 4; i++) {
			...
			if (arcProps->GetArcProp(level, propIDs[i], &prop) !=
			    S_OK)
				continue;
			if (prop.vt != VT_EMPTY) {
				UString* s = NULL;
		...
		{
			NCOM::CPropVariant prop;
			if (arcProps->GetArcProp(level, kpidErrorFlags,
						 &prop) == S_OK) {
				UInt32 flags = GetOpenArcErrorFlags(prop);
				if (flags != 0)
					lev.ErrorFlags =
					    GetOpenArcErrorMessage(flags);
			}
		}
...
static void GetFolderError(CMyComPtr<IFolderFolder>& folder,
			   UString& open_Errors, UString& nonOpen_Errors)
{
	CArcLevelsInfo levs;
	...
	FOR_VECTOR(i, levs.Levels)
	{
		...
		const CArcLevelInfo& lev =
		    levs.Levels[levs.Levels.Size() - 1 - i];

		UString m;

		if (!lev.ErrorType.IsEmpty()) {
			m = MyFormatNew(IDS_CANT_OPEN_AS_TYPE,
					GetBracedType(lev.ErrorType));
			if (!isNonOpenLevel) {
				m.Add_LF();
				m += MyFormatNew(IDS_IS_OPEN_AS_TYPE,
						 GetBracedType(lev.Type));
			}
		}
...
HRESULT CFfpOpen::OpenFileFolderPlugin(IInStream* inStream, const FString& path,
				       ...
				       HWND parentWindow)
{
	...
	*/
	...
	/*
	else if (library.LoadAndCreateManager(plugin.FilePath, plugin.ClassID,
	&t.FolderManager) != S_OK) continue;
	*/
	...
	*/
	...
	/* COpenCallbackImp object will exist after Open stage for multivolume
	 * archives */
	...
	/*
	  ...
	  from each volume object (CInFileStreamVol) will be closed (when
	  archive will be closed).
	*/
	t.OpenCallbackProgress.Release();

	if (t.Result != S_FALSE && t.Result != S_OK) {
		// Archive manager returned an error (e.g. E_NOTIMPL for unknown
		// format). Try LKL plugin before giving up.
		CMyComPtr<IFolderManager> lklManager = new CLklFolderManager;
		CMyComPtr<IFolderFolder> lklFolder;
		HRESULT hr = lklManager->OpenFolderFile(
		    inStream, fs2us(path), arcFormat, &lklFolder, NULL);
		if (hr == S_OK && lklFolder) {
			Folder = lklFolder;
			return S_OK;
		}
		return t.Result;
	...
	if (t.Folder) {
		...
		if (!nonOpen_Errors.IsEmpty()) {
			...
			/* if there are good open leves, and non0open level,
			   we could force error as critical error and return
			   error here but it's better to allow to open such
			   rachives */
			// return S_FALSE;
		}
	}
	...
	// Archive manager failed — try LKL folder manager for disk images
	{
		CMyComPtr<IFolderManager> lklManager = new CLklFolderManager;
		CMyComPtr<IFolderFolder> lklFolder;
		HRESULT hr = lklManager->OpenFolderFile(
