// FilePlugins.cpp (patched for LKL)
	...
	*/
	{
		...
		else
		{
		  if (!plugin.ClassID_Defined)
		    continue;
		  if (pluginLib.LoadAndCreateManager(plugin.FilePath,
		plugin.ClassID, &folderManager) != S_OK) continue;
		}
		*/
		CMyComBSTR extBSTR;
		...
		FOR_VECTOR(i, exts)
		{
			...
			if (folderManager->GetIconPath(ext, &iconPath,
						       &iconIndex) == S_OK)
				if (iconPath) {
					plugPair.IconPath =
	...
	// Register LKL disk image extensions
	{
		CMyComPtr<IFolderManager> lklManager = new CLklFolderManager;
		CMyComBSTR extBSTR;
		if (lklManager->GetExtensions(&extBSTR) == S_OK) {
			UStringVector exts;
			SplitString((const wchar_t*)extBSTR, exts);
			FOR_VECTOR(i, exts)
			{
