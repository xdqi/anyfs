...
static void usage(const char* prog)
{
	fprintf(
	    ...
	    "Supports ext2/3/4, xfs, btrfs, FAT/exFAT, NTFS, squashfs, "
	    "ISO9660, etc.\n\n"
	    "Options:\n"
	    ...
	    "Shell commands:\n"
	    "  open <image>       Open disk image (raw, qcow2, vmdk, vdi, "
	    "vhd)\n"
	    "  mount [part]       Mount filesystem (auto-detect type)\n"
	    "  ls/ll [path]       List directory / long listing\n"
	    "  cat/head/tail      Print file contents\n"
	    "  stat <path>        Show file information (size, mode, times)\n"
	    "  hexdump <path>     Hex dump of file\n"
	    "  find [path]        Recursive file listing\n"
	    "  download <r> <l>   Extract file to local filesystem\n"
	    "  df                 Show disk space usage\n"
	    "  cd/pwd/lcd         Navigate directories (guest/host)\n"
