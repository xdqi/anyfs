...
# kernel.config overlay — re-applied on every .config rebuild by
# tools/lkl/Makefile (defconfig → append kernel.config → olddefconfig).
# This is the canonical override set — the cfg -d/-e lines above are just for
# the initial .config; tools/lkl wipes them on its next build cycle, so any
# override we actually care about MUST appear here.
cat > "$LKL_OUT/kernel.config" <<'EOF'
# Anyfs-reader wasm overlay — re-applied on every .config rebuild.
...
# Device Mapper / LUKS / LVM — needed by anyfs container recursion (v2).
CONFIG_MD=y
CONFIG_BLK_DEV_DM=y
CONFIG_DM_CRYPT=y
...
# Crypto primitives (dm-crypt for LUKS).
CONFIG_CRYPTO=y
CONFIG_CRYPTO_AES=y
CONFIG_CRYPTO_XTS=y
...
# Filesystems: journaling / COW. XFS_FS works on wasm once
# patches/linux/wasm/01-xfs-this-address-wasm.patch is applied
# (handled by oot_fs.sh stage --wasm).
...
# Network servers — off (no networking on wasm).
# CONFIG_NFSD is not set
# CONFIG_SMB_SERVER is not set

# Unicode / NLS — full codepage set for FAT/exFAT/NTFS non-ASCII names.
CONFIG_UNICODE=y
CONFIG_NLS_UTF8=y
