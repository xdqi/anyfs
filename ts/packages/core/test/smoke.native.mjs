#!/usr/bin/env node
// Smoke test for NativeSession against the real @anyfs/native addon —
...
// Wrap the synchronous addon as the async bridge NativeSession expects.
// Real Electron preload.ts does the same thing over ipcRenderer.invoke.
const bridge = {
    available: async () => true,
    init: async (m, l) => addon.kernelInit(m, l),
    diskOpen: async (p, f) => addon.sessionOpen(p, f),
    diskClose: async (h) => addon.sessionClose(h),
    diskListJson: async (h) => addon.sessionListJson(h),
    diskMetaJson: async (h) => addon.sessionMetaJson(h),
    diskEnter: async (h, p, f) => addon.sessionEnter(h, p, f),
    mountWhole: async (h, fs, f) => addon.sessionEnter(h, 0, f),
...
try {
    ...
    console.error('[3/6] meta + listParts');
    ...
    let st;
    try {
        st = await session.statFollow(target);
    } catch (e) {
        ...
        st = await session.statFollow(`${mp}/${f.name}`);
    }
    console.error(`     size=${st.size} mode=0${(st.mode & 0o777).toString(8)}`);

    console.error('[6/6] close');
    await session.close();
